...
CPU_LIMIT="500m"
DEPLOY_DB_VARS=true
EXTERNAL_ENV_FILE="/home/smartpds/k8s-smartpds/apigateway/external-var-apigateway.txt"  # Path to external file
  ...
  if [[ -f "$EXTERNAL_ENV_FILE" ]]; then
    while IFS= read -r line; do
      ...
      if [[ -n "$line" && ! "$line" =~ ^# ]]; then
        ...
        ENV_NAME=$(echo "$ENV_NAME" | tr '.' '_' | tr '[]' '_' | awk '{print toupper($0)}')
